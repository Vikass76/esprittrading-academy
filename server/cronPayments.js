    ...
    if (newRetryCount > MAX_RETRIES) {
      ...
      await sendEmail(payment.email, 'Action requise - ton accès à la formation OTE 705',
        `<p>Nous n'avons pas pu prélever ton 2e versement de ${(payment.amount_due/100).toFixed(2)}€ après plusieurs tentatives.</p>
         ...
         <a href="https://esprittrading.fr/checkout.html?retry=${payment.id}" style="display:inline-block;background:#F4C70F;color:#000;padding:12px 24px;border-radius:8px;text-decoration:none;font-weight:700;margin:16px 0">Relancer mon paiement</a>`);
